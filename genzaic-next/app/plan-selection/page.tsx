...
import {
  ...
  Heart, Megaphone, LayoutGrid, Plug, Globe, Boxes, Info, Loader2,
} from "lucide-react"
          ...
          <div className="grid gap-6 md:grid-cols-2 mb-8">
            ...
            >
              ...
              <div className="mb-6">
                ...
                <h2 className="text-2xl font-bold mb-1">Creator Plan</h2>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-4xl font-bold">₹0</span>
                  <span className="text-muted-foreground">Setup Fee</span>
                </div>
                <div className="inline-block px-3 py-1 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 text-sm font-semibold">
                  5% Platform Fee
                </div>
              </div>
                ...
                {creatorBenefits.map((benefit, index) => (
                  <li key={index} className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center shrink-0">
                      <benefit.icon className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                    </div>
                    <span className="flex-1 text-sm">{benefit.text}</span>
                    <Tooltip>
            ...
            <div className="relative p-6 pt-8 rounded-2xl border-2 border-border bg-card overflow-visible">
              ...
              {/* Overlay */}
              <div className="absolute inset-0 bg-background/30 z-10 pointer-events-none rounded-2xl" />

              <div className="mb-6">
                <div className="w-14 h-14 rounded-xl gradient-primary flex items-center justify-center mb-4">
                  <TrendingUp className="w-7 h-7 text-white" />
                </div>
                <h2 className="text-2xl font-bold mb-1">Startup Plan</h2>
                <div className="flex items-baseline gap-2 mb-2">
                ...
                {startupBenefits.map((benefit, index) => (
                  <li key={index} className="flex items-center gap-2 text-foreground/70">
                    <div className="w-5 h-5 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center shrink-0">
                      <benefit.icon className="w-3 h-3 text-purple-600 dark:text-purple-400" />
                    </div>
                    <span className="flex-1 text-sm">{benefit.text}</span>
                    <Tooltip>
                      <TooltipTrigger asChild>
              ...
              {isProcessing ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-lg font-medium">We&apos;re getting things ready for you...</span>
                  </div>
                ...
                >
                  <Button
                    ...
                    className="gap-2 px-12 gradient-primary text-white"
                  >
