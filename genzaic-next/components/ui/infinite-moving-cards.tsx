...
export function InfiniteMovingCards({
  ...
  const getSpeed = useCallback(() => {
    if (containerRef.current) {
      const d = speed === "fast" ? "20s" : speed === "normal" ? "40s" : "80s"
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li
            className="w-[350px] max-w-full relative rounded-2xl border border-border/50 flex-shrink-0 px-8 py-6 md:w-[450px] bg-card"
            key={item.name + idx}
          >
            <blockquote>
